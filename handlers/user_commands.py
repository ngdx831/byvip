...
from config.settings import (
    ...
    ADMIN_USER_IDS, VIP_DURATION_DAYS, PAYMENT_TIMEOUT_MINUTES,
)
...
from database.operations import (
    ...
    get_pending_payments, expire_user_pending_payments,
)
...
async def cmd_pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    old_pending_payments = get_pending_payments(user_id)
    for payment in old_pending_payments:
        checker.cancel_monitoring(payment["id"])
    if old_pending_payments:
        expire_user_pending_payments(user_id)
...
⚠️ 注意：
• 金额不匹配将无法自动到账
• {PAYMENT_TIMEOUT_MINUTES}分钟内未支付订单将自动取消
• 如有问题请联系客服
"""
    await update.message.reply_text(message, parse_mode="HTML")
